            ...
            {links && links.length > 0 ?
                links.map((link, i) => {
                    return (i === links.length - 1) ?
                        <a key={i} className={`text-uppercase ${linkClass} ${link.path === currentPath ? 'text-white font-weight-bold' : 'text-gray'}`}
                              href={link.path}>{link.name}</a> :
                        <>
                            <a key={i} className={`text-uppercase text-gray ${linkClass}`}
